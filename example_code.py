...
# --- STATE MANAGEMENT ---
system_active = False  # This flag controls if the tracking/shooting is running
still_time = time.perf_counter()


print("System Standby. Click the switch once to START calibration/tracking.")


try:
    while True:
        # 1. Check if we need to start the system
        if not system_active:
            if GPIO.input(SWITCH_PIN) == GPIO.LOW: # Button pressed
                print("Button Pressed! Calibration mode ACTIVATED.")
                system_active = True
                still_time = time.perf_counter() # Reset timer right at start
                time.sleep(0.5) # Debounce delay


        # 2. Only run tracking if system is active
        if system_active:
