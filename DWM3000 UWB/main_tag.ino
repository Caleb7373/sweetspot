...
// WiFi Configuration
const char *ssid = "Claires iPhone";
const char *password = "k42gn6d7afdfr";
const char *host = "172.20.10.9"; // Your PC's IP address
const int port = 7007;             // Choose a port number
WiFiClient client;
bool wifiConnected = false;
int lastStage = 0;
#define MAX_RX_ATTEMPTS 10000
static int rx_attempts = 0;
// const short MAX_RETRIES = 100;
// short curr_retries = 0;
...
void loop()
{
    ...
    if (rx_status = DWM3000.recedfivedFrameSucc())
    {
        rx_attempts = 0;
        DWM3000.clearSystemStatus();
        if (rx_status == 1)
        ...
        {
            Serial.print("[ERROR] RX Error from Anchor ");
            Serial.println(currentAnchorId);
            DWM3000.clearSystemStatus();
            curr_stage = 0;  // ← also reset here, was missing
        }
    }
    else if (++rx_attempts > MAX_RX_ATTEMPTS)
    {
        Serial.print("[TIMEOUT] No response from Anchor ");
        Serial.println(currentAnchorId);
        rx_attempts = 0;
        switchToNextAnchor();  // don't get stuck on a dead anchor
        curr_stage = 0;
    }
    break;
    // case 1: // Await first response
    ...
    //     }
    //     // else {
    //     //     // only try _ times and then go to case 0
    //     //     curr_retries++;
    //     // }
    //     // Serial.println("7");
    //     break;
    ...
    {
        Serial.print("[TIMEOUT] No report from Anchor ");
        Serial.println(currentAnchorId);
        rx_attempts = 0;
        switchToNextAnchor();
        curr_stage = 0;
    ...
    //     else
    //     {
    //         Serial.println("Awaiting second response");
    //     }
    //     break;
    ...
    //     //Serial.println(curr_retries);
    //     Serial.println("MAX_RETRIES exceeded. Going to stage 0");
    //     curr_stage = 0;
    //     curr_retries = 0;
